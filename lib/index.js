...
Tracker.prototype.getAppDataRootFolder = function() {
	var result = path.join(process.env.APPDATA || (process.platform === "darwin" ? process.env.HOME + "Library/Preferences" : "/var/local"), "time-track");
	if (this.program.testMode){
...
Tracker.prototype.reportByDay = function(date) {
	...
	if (result >= restSecondsToWorkThisDay){
		...
		if (restSecondsToWorkThisDay > 0 && result > 0){
			...
			this.log(color("Time elapsed:"), color(date), color("-"), color(time.format("HH:mm:ss")));
		}
	} else {
